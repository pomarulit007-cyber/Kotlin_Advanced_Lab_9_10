...
### Геттеры и сеттеры
Используются для контроля доступа к свойствам объектов. Позволяют добавлять валидацию, логирование и вычисляемые значения.

**Пример:**
```kotlin
var health: Int = 100
    set(value) {
        field = value.coerceIn(0, 100) // Ограничение диапазона
    }
